...
import {
	...
	TableCell,
	TableBody,
	Collapse, Accordion, AccordionSummary, AccordionDetails, Box, Stack, Avatar,
} from '@mui/material';
			...
			<Accordion expanded={isExpanded} onChange={()=>{
				...
				<AccordionSummary expandIcon={<ExpandMoreIcon />}>
					<Stack>
						...
						<Accordion className={"outerAccordion"} disableGutters expanded={!isExpanded} onChange={()=>{
							setIsExpanded(!isExpanded)
						}}>
							<AccordionSummary style={{height:0,minHeight:0}}></AccordionSummary>
							<AccordionDetails>
								<Grid container spacing={2}>
...
function TeamCard({teamName,positions}:{teamName: string; positions: string[];}) {
	// @ts-ignore
	return (
			<Grid item xs={4}>
...
function TeamCardSecondary({teamName,positions}:{teamName: string; positions: string[];}) {
	// @ts-ignore
	return (
		<Grid item xs={4}>
			<Card variant="outlined">
...
function EventPosition({position, volunteer}:{position: string;volunteer:string}) {
	...
	return (
		<TableRow
			key={position}
			className={styles} onDragLeave={(e) => {
			e.preventDefault()
			setStyle('')
		}} onDragEnter={(e) => {
			e.preventDefault()
...
function VolunteerCard({volunteers}:{ volunteers: string[];}) {
	...
	useEffect(() => {
		const mArr = Array.from({length: 99}, (_, i) => `/public/img/profile-pics/man-${i+1}.jpg`);
		const wArr = Array.from({length: 99}, (_, i) => `/public/img/profile-pics/woman-${i+1}.jpg`);
